...
def pickup(dims, path, savepath):
    ...
    with open(config_file, 'r') as f:
        for line in f.readlines():
            ...
            if line.startswith('absolute_save_path'):
                args['absolute_save_path'] = os.path.abspath(
                    savepath)
                arg_str += '\nabsolute_save_path {}\n'.format(
                    args['absolute_save_path'])
            arg_str += line
            chunks = line.split()
            if len(chunks) < 2:
                continue
    ...
    ae = tf.keras.models.load_model(path)
    return ae, args, arg_str
...
def main():
    ...
    if args.pickup is None:
        ...
        savepath = os.path.abspath(
            os.path.join(args.save_path, str(int(time.time()))))
        ae, _, arg_str = pickup((28, 48, 48, 48), args.pickup, savepath)
